...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Create Base44 client from request (handles user auth automatically)
    ...
    // Fetch film using service role (bypasses RLS)
    ...
    const rental = await base44.asServiceRole.entities.FilmRental.create({
      user_id: user.id,
      film_id: film_id,
      status: 'pending'
    });
